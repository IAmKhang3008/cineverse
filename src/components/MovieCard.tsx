...
import { usePosterOptimizer } from "@/hooks/usePosterOptimizer";
...
export default function MovieCard({ movie, fromSearch, onHoldChange, rating }: MovieCardProps) {
  ...
  // Lấy poster tối ưu — luôn trả về { type: 'direct', url }
  // imageService đã xử lý proxy, MovieCard chỉ cần dùng url trực tiếp
  const { imageData, isLoading } = usePosterOptimizer(
    movie?.slug || '',
    movie?.poster_url || movie?.thumb_url || '',
  );

  // Reset error khi nguồn ảnh thay đổi
  useEffect(() => {
    setImgError(false);
  }, [imageData?.url]);
  ...
  const handleTouchStart = useCallback(() => { setActive(true);  }, [setActive]);
  const handleTouchEnd   = useCallback(() => { setActive(false); }, [setActive]);
  ...
  // URL ảnh cuối cùng — luôn là string đã proxy, không cần xử lý thêm
  const posterUrl = (!imgError && imageData?.url) ? imageData.url : null;
  const showSkeleton = isLoading || (!posterUrl && !imgError);
  ...
  return (
    ...
    >
      ...
      >
        {/* Ảnh poster — một nhánh duy nhất, không phân biệt tmdb/direct */}
        {posterUrl ? (
          <img
            src={posterUrl}
            alt={movie.name || ''}
          ...
          /* Skeleton / fallback khi chưa load xong hoặc lỗi */
