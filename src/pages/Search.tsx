...
export default function Search() {
  ...
  const fetchSearch = useCallback(async () => {
    if (!query && !filters.category && !filters.country && !filters.year) {
      setMovies([]);
      setResultCount(0);
      setTotalPages(1);
      setLoading(false);
    ...
    try {
      ...
      setTotalPages(res.pagination?.totalPages || Math.ceil(totalItems / 64) || 1);
  ...
  return (
    <motion.div 
      ...
      className="max-w-[1280px] mx-auto px-4 sm:px-6 py-8 md:py-12 mt-16 text-white"
    >
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-heading font-bold text-white tracking-wide">
              {query ? `Kết quả cho: "${query}"` : 'Tìm kiếm và Khám phá'}
            </h1>
          </div>
          {!loading && (
      ...
      {!loading && movies.length > 0 && (
        <>
          <motion.div
            key={`${page}-${query}-${JSON.stringify(filters)}`}
          ...
          >
            {movies.map((movie, index) => (
              <motion.div key={`${movie.slug || movie._id || 'search'}-${index}`} variants={itemVariants}>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center mt-12 md:mt-16 gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-2 md:px-4 md:py-2 bg-[#2A2A2A] rounded-lg disabled:opacity-30 hover:bg-[#333] text-sm md:text-base transition-colors"
              >
                Trước
              </button>
              {getPageNumbers().map(p => (
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 md:w-10 md:h-10 rounded-lg font-bold text-sm md:text-base transition-colors ${p === page ? "bg-[#E50914]" : "bg-[#2A2A2A] hover:bg-[#333]"}`}
                >
                  {p}
                </button>
              ))}
      ...
      {!loading && movies.length === 0 && (
        <div className="text-center text-[#A0A0A0] py-16 md:py-20 backdrop-blur-xl bg-white/[0.03] border border-white/10 rounded-3xl px-4 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
          ...
          <p className="text-xs md:text-sm mt-2 mb-6">Vui lòng thay đổi bộ lọc hoặc từ khóa.</p>
          <div className="flex flex-wrap justify-center gap-2 md:gap-3">
            {["Hành Động", "Viễn Tưởng", "Hàn Quốc", "Marvel", "Anime"].map(suggestion => (
              <button
                key={suggestion}
