...
const oldSearchBlock = `export async function searchTmdbWithCache(movie: any) {
...
const newSearchBlock = `export async function searchTmdbWithCache(movie: any) {
  ...
  const resolveSeasonFromTmdb = async (tmdbId, mediaType) => {
  ...
  // 3. Search by title
  const searchYear = String(movie.year || '');
  const cacheKey = \`tmdb_unified_search_v4_\${movie.slug || searchOrigin || searchName}_\${searchYear}\`;
  return fetchWithCache(cacheKey, async () => {
    let finalResult = null;
    // Await sequentially to prioritize original name over localized name
    if (searchOrigin) {
